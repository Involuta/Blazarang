...
@export var rotation_y_correction := 0.0 # Angle added to mesh rotation y
@export var rotation_x_correction := PI/2 # Angle added to mesh rotation x
...
func lateral_look_at_target(turn_speed):
	...
	rotation.y = move_toward(rotation.y, rotation_y_correction + target_global_rotation.y, turn_speed)
...
func vert_look_at_target(turn_speed):
	...
	mesh.rotation.x = move_toward(mesh.rotation.x, rotation_x_correction + target_global_rotation.x, turn_speed)
...
func vert_look_high_bounce_trajectory(turn_speed):
	...
	mesh.rotation.x = move_toward(mesh.rotation.x, rotation_x_correction + target_global_rotation_x, turn_speed)
