...
var disappear_time := 6.0 # Time spent waiting for particles to disappear after ring disappears
...
func ring_animation():
	...
	var t = create_tween()
	t.tween_property(mat, "shader_parameter/progress", 1.0, appear_time).from(0.0).set_ease(Tween.EASE_IN_OUT)
